...
class Employee(User):
    ...
    def search_item(self, stock: List[Warehouse], search_term: str) -> Tuple[List[Item], Counter]:
        found_items = []
        item_counts = Counter()
        ...
        if found_items:
            while True:
                ...
                for i, item in enumerate(found_items, 1):
                    ...
                    print(f"{i}. {item_key}, Days in Stock: {days} days , Available: {count} pcs")
...
class SessionReport:
    ...
    def record_ordered_item(self, item_state: str, item_category: str, quantity: int) -> None:
