...
class Item:
    def __init__(self, state, category, date_of_stock, warehouse):
        self.state = state
...
class Warehouse:
    ...
    def search(self, search_term):
        return [item for item in self.stock if search_term.lower() in item.category.lower()]
...
class User:
    ...
    def browse_by_category(self, stock):
        categories = {}
        ...
        try:
            ...
            if 1 <= category_choice <= len(categories):
                ...
                self.last_browsed_quantity = categories[selected_category]  # Set the last browsed quantity
                ...
                for item in found_items:
                    print( f"{item.state} ({item.category}) - Stocked on: {item.date_of_stock}")
            else:
...
class Employee(User):
    ...
    def search_and_order_item(self, stock):
        ...
        while True:
            search_term = input("Enter the item you want to search (or 'cancel' to go back): ").strip()
    ...
    def search_item(self, stock, search_term):
        found_items = []
        item_counts = {}
        ...
        for warehouse in stock:
            for item in warehouse.stock:
                if search_term.lower() in item.category.lower():
                    ...
                    self.last_searched_item = search_term
                    

        return found_items, item_counts

    def order_items(self, found_items, item_counts):
        if found_items:
            while True:
                ...
                for i, item in enumerate(found_items, 1):
                    ...
                    print(f"{i}. {item_key}, Date of Stock: {item.date_of_stock}, Available: {count}")

                item_choice = input("Enter the number of the item you want to order (or 'cancel' to go back): ").strip()
                ...
                try:
                    ...
                    if 1 <= item_choice <= len(found_items):
                        ...
                        print(f"You have selected: {item_key}, Available: {available_quantity}")

                        while True:
                            order_quantity = input("Enter the quantity you want to order: ").strip()
                            if order_quantity.isdigit():
                                order_quantity = int(order_quantity)
                                if 1 <= order_quantity <= available_quantity:
                                    self.place_order(selected_item, order_quantity, item_counts)
                                    
                                    self.last_ordered_item = selected_item  # Set the last browsed item
                                    self.last_ordered_quantity = order_quantity # Set the last browsed quantity 
    ...
    def place_order(self, item, quantity, item_counts):
        item_key = f"{item.state} {item.category} (Warehouse {item.warehouse})"
        available_quantity = item_counts.get(item_key, 0)
...
class SessionReport:
    ...
    def record_searched_item(self, item_name):
        self.items_searched.append(item_name)

    def record_browsed_item(self, item_name):
        self.items_browsed.append(item_name)

    def record_ordered_item(self, item_state, item_category, quantity):
