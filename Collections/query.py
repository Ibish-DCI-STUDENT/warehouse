...
def search_and_order_item(item_name):
    ...
    if found_items:
        ...
        for item in found_items:
            ...
            print(
                f"- Warehouse {item['warehouse']} (in stock for {days_in_stock} days)"
            )

        # Check if the item is in more than one warehouse
        warehouse_counts = {}
        for item in found_items:
            warehouse_counts[item["warehouse"]] = (
                warehouse_counts.get(item["warehouse"], 0) + 1
            )
        ...
        if len(max_warehouses) == 1:
            ...
            print(
                f"Maximum availability: {max_count} in Warehouses {', '.join(map(str, max_warehouses))}"
            )
